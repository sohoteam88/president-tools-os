...
export function WhyStorySession() {
  ...
  const [answers, setAnswers] = useState<string[]>(Array.from({ length: WHY_STORY_QUESTIONS.length }, () => ""));
  ...
  async function extractMoments() {
    ...
    setIsLoading(true);
    setStep("extracting");
    const response = await fetch(`/api/voice/why-story/${sessionId}/extract`, { method: "POST" });
    const body = (await response.json().catch(() => ({}))) as { draftMoments?: DraftMoment[] };
    setIsLoading(false);
    ...
    setSelectedIndices(body.draftMoments.map((moment) => moment.questionIndex));
    setStep("confirming");
  }
    ...
    return (
      <section className="rounded-md border border-border bg-card p-5">
        ...
        <ol className="mt-4 list-decimal space-y-2 pl-5 text-sm text-muted-foreground">
          {WHY_STORY_QUESTIONS.map((question) => <li key={question}>{question}</li>)}
        </ol>
        <button
          type="button"
      ...
      <section className="rounded-md border border-border bg-card p-5">
        ...
        <div className="mt-4 space-y-3">
          {draftMoments.map((moment) => {
            ...
            const typeLabel = typeKey ? t[typeKey] : MOMENT_TYPE_LABELS[moment.momentType];
            return (
              <label key={moment.questionIndex} className="flex items-start gap-3 rounded-md border border-border p-3">
                <input
                  type="checkbox"
                  ...
                  onChange={(event) => {
                    setSelectedIndices((current) =>
                      event.target.checked
                        ? [...new Set([...current, moment.questionIndex])]
                        : current.filter((index) => index !== moment.questionIndex)
                    );
                  }}
                  className="mt-1"
                />
  ...
  if (step === "done") {
    return (
      <section className="rounded-md border border-border bg-card p-8 text-center">
        ...
        <button type="button" onClick={() => setStep("intro")} className="mt-5 rounded-md border border-border px-4 py-2 text-sm font-medium">
          {t.recordAgain}
        </button>
      </section>
    );
  ...
  if (step === "extracting") {
    return <p className="rounded-md border border-border bg-card p-8 text-center text-sm text-muted-foreground">{t.aiAnalysing}</p>;
  }

  return (
    <section className="rounded-md border border-border bg-card p-5">
      <p className="text-sm font-medium">{t.questionLabel} {currentQuestion + 1} / {WHY_STORY_QUESTIONS.length}</p>
      <h2 className="mt-2 text-base font-semibold">{WHY_STORY_QUESTIONS[currentQuestion]}</h2>
      <textarea
        ...
        rows={5}
        className="mt-4 w-full rounded-md border border-input bg-background px-3 py-2 text-sm leading-6"
      ...
      <div className="mt-4 flex gap-2">
        {currentQuestion > 0 ? (
          <button
            type="button"
            onClick={() => setCurrentQuestion((currentQuestion - 1) as WhyStoryQuestionIndex)}
        ...
        {currentQuestion < WHY_STORY_QUESTIONS.length - 1 ? (
          <button
            type="button"
            onClick={() => void saveAnswer((currentQuestion + 1) as WhyStoryQuestionIndex)}
            disabled={isLoading}
